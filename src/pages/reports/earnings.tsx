...
export default function EarningsReport({ appointments }: Props) {
    ...
    appointments.forEach((a) => {
      if (a.services && a.services.length > 0) {
        a.services.forEach((s) =>
          set.add(s.service?.name || "Service deleted"),
        );
      } else {
        set.add("Service (deleted)");
      }
    });
  ...
  const filtered = useMemo(() => {
    ...
    return chartData.filter((r) => {
      ...
      return (
        d.isAfter(s.subtract(1, "second")) && d.isBefore(e.add(1, "second"))
      );
    });
  }, [chartData, dateRange]);
    ...
    <Stack gap="md">
      <Group justify="space-between">
        ...
        <Group>
          <Button
            ...
            onClick={() => exportCSV(`${filePrefix}.csv`, csvHeaders, csvRows)}
          >
            Export CSV
          </Button>
          <Button
          ...
          <Button
            ...
            onClick={() =>
              printPDF("Service Earnings Report", csvHeaders, csvRows)
            }
          >
            Print PDF
          </Button>
      ...
      <Paper withBorder p="md">
        <BarChart
          ...
          dataKey="date"
          type="stacked"
          series={serviceNames.map((name, i) => ({
            name,
            color: `teal.${(i + 3) % 10}`,
          }))}
