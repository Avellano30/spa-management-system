...
export default function AppointmentsReport({ appointments }: Props) {
  // Helper to get all service names as a string
  ...
  const filteredAppointments = useMemo(() => {
    return appointments.filter((appt) => {
      ...
      if (start && end) {
        ...
        if (
          !apptDate.isAfter(s.subtract(1, "second")) ||
          !apptDate.isBefore(e.add(1, "second"))
        )
          return false;
      }
      ...
      if (
        ...
        !(
          appt.services &&
          appt.services.some((s) => s.service?.name === serviceFilter)
        )
      )
        return false;
  ...
  const clientOptions = Array.from(
    new Set(appointments.map(getClientName)),
    ...
    new Set(
      appointments.flatMap((a) =>
        a.services && a.services.length > 0
          ? a.services.map((s) => s.service?.name || "Service deleted")
          : ["Service (deleted)"],
      ),
    ),
  ).map((name) => ({ value: name, label: name }));
      ...
      <Group justify="space-between">
        ...
        <Group>
          <Button
            ...
            onClick={() =>
              exportCSV("appointments-status-report.csv", csvHeaders, csvRows)
            }
          >
            Export CSV
          </Button>
          <Button
            ...
            onClick={() =>
              exportPDF(
                "Appointments Status Report",
                "appointments-status-report.pdf",
                csvHeaders,
                csvRows,
              )
            }
          ...
          <Button
            ...
            onClick={() =>
              printPDF("Appointments Status Report", csvHeaders, csvRows)
            }
          >
            Print PDF
          </Button>
            ...
            <Table.Tbody>
              {filteredAppointments.map((appt) => (
                <Table.Tr key={appt._id}>
                  ...
                  <Table.Td>{getServiceNames(appt)}</Table.Td>
