    ...
    appointments.forEach((a) => {
      if (a.services && a.services.length > 0) {
        a.services.forEach((s) => {
          const name = s.service?.name || "Service deleted";
          map.set(name, (map.get(name) ?? 0) + 1);
        });
      } else {
        map.set("Service (deleted)", (map.get("Service (deleted)") ?? 0) + 1);
      }
    });
      ...
      <Group justify="space-between">
        ...
        <Group>
          <Button
            ...
            onClick={() =>
              exportCSV("most-requested-services.csv", headers, rows)
            }
          >
            Export CSV
          </Button>
